...
표기 규칙(2026-08-04 개정): 한글 표기는 **"도돌이"**(런처 앱 이름·app.json `name`), 로마자·인앱 워드마크·웹 타이틀은
**Dodori**(첫 글자만 대문자, `web.name`·`web.shortName`). "도도리"는 폐기 — 마크의 유래인 음악 기호 "도돌이표"와 결이 맞는 쪽으로 돌아왔다.
소문자 `dodori`는 **기술 식별자에만** 남는다(`slug`·`scheme`·번들 ID) — 바꾸면 EAS 연결이 끊긴다.
