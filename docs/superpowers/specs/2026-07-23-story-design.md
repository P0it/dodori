...
| 질문 | 결정 |
|---|---|
| 앨범 연동 방식 | **올린 날짜로 자동 연결.** 그날 트랙이 있으면 붙고, 없으면 그냥 일상 스토리 |
...
- `track_id`는 클라이언트가 올릴 때 채운다 — `tracks`에서 `date = todayKST()`인 행이 있으면 그 id, 없으면 null
...
| 올리기 | `modals/create-story` | 사진 1장 → 캡션(선택) → 올리기. 그날 앨범이 있으면 "○○에 함께 담겨요" 한 줄 |
