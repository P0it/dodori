...
### 지도 위 컨트롤 — 뒤로가기만 남기고 나머지는 시트 헤더로

`TopBar`는 배경색이 없고 tint가 흰색이라 밝은 네이버 지도 위에서 글자가 사라졌다.
처음엔 셋(뒤로가기 / 리스트명 / 수정)을 전부 불투명 알약으로 지도 위에 띄웠는데,
떠 있는 요소가 셋이나 되니 지도를 가리고 산만했다.

**지도 위엔 뒤로가기 원형 버튼 하나만** 남긴다 (`useSafeAreaInsets()`로 상태바를 피함).
리스트명과 수정 버튼은 **시트 헤더**로 옮긴다 — 시트는 항상 화면에 있으므로 제목이 사라지지 않고,
지도는 깨끗해진다.

시트 헤더는 `BottomSheetFlatList` **바깥에** 고정한다. 안에 넣으면 목록과 함께 스크롤돼 사라진다.
구성은 리스트 타일(40px) + 이름 + `장소 N곳` + `수정`/`완료`.
수정 중에는 타일·이름이 `draft`를 따라가므로 이 헤더가 그대로 미리보기가 된다 —
그래서 편집 UI 안에 따로 두었던 64px 미리보기 타일은 뺐다(한 화면에 같은 타일이 둘 뜨는 것을 피한다).
